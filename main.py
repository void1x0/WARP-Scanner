...
def main_v6():
    global which, ping_range, save_best, resultss
    resultss = []
    def generate_ipv6():
    ...
    for ip, port, ping, loss_rate, jitter, combined_score in sorted_results:
        ...
        if which == '3' and do_you_save == '1':
            if need_port == "2":
                ...
                save_best.append('[' + ip + ']' + str(port) + ' | ' + 'ping: ' + str(ping) + 'packet_lose: ' + str(loss_rate) + 'jitter: ' + str(jitter) + '\n')
...
def main():
    global which, max_workers_number, ping_range, results, save_result
    ping_range = ''
    results = []
    if do_you_save == '1':
    ...
    for ip, port, ping, loss_rate, jitter, combined_score in sorted_results:
        ...
        if which == '3' and do_you_save == '1':
            if need_port == "2":
                ...
                save_result.append(ip + str(port) + ' | ' + 'ping: ' + str(ping) + 'packet_lose: ' + str(loss_rate) + 'jitter: ' + str(jitter) + '\n')
    console.clear()
    table = Table(show_header=True, title="IP Scan Results", header_style="bold blue")
    table.add_column("IP", style="dim", width=15)
    table.add_column("Port", justify="right")
...
def main3():
    global best_result, wire_config_temp, wire_c, wire_p
    if wire_p == 0:
         try:
             best_result = main()
         except Exception:
